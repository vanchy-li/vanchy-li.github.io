...
Promise.all([d3.csv(STUDY_PATH), d3.csv(CLIMATE_PATH), d3.json(TRAPS_PATH), d3.json(BOUNDARY_PATH), d3.csv(LAYER3_PATH)])
  .then(([studyRows, climateRows, trapGeojson, boundaryGeojson, layer3Rows]) => {
...
function renderMap() {
  const points = traps.features;
  const polygon = boundary.features[0].geometry.coordinates[0];
  Plotly.react("trap-map", [
    {
      type: "scattermapbox", mode: "lines", name: "Study boundary",
      lon: polygon.map(d => d[0]), lat: polygon.map(d => d[1]),
      fill: "toself", fillcolor: "rgba(23,107,99,.10)",
      line: { color: COLORS.teal, width: 2 }, hoverinfo: "skip"
    },
    {
      type: "scattermapbox", mode: "markers", name: "Ovitrap",
      lon: points.map(d => d.geometry.coordinates[0]),
      lat: points.map(d => d.geometry.coordinates[1]),
      text: points.map(d => d.properties.Name),
      marker: { size: 12, color: COLORS.orange, opacity: .95 },
      hovertemplate: "<b>%{text}</b><br>Ovitrap location<extra></extra>"
    }
  ], {
    paper_bgcolor: COLORS.white, margin: { t: 0, r: 0, b: 0, l: 0 }, showlegend: false,
    mapbox: { style: "carto-positron", center: { lon: -64.872, lat: -24.234 }, zoom: 12.1 }
